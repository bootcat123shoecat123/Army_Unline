...
@export_multiline var message:String = "ただ、所々%ruby{丹塗,にぬり}の%ruby{剥,は}げた、\n大きな%ruby{円柱,まるばしら}に、\n%ruby{蟋蟀,きりぎりす}が一匹とまっている"
@export var SPACING:int = 24
@export var SPACING_EXTRA:int = 3
...
func apply():
	...
	var m_font: FontFile = m_label.get("custom_fonts/font")
	for ruby in rubies:
		# ルビ用Labelを複製
		var r_label: Label = $_Ruby.duplicate()
		var r_font: FontFile = r_label.get("theme_font_override")
		r_label.set("theme_font_override", r_font)
		# ルビを指定
		...
		var m_pre_after_nl_idx: int = m_pre_text.rfind("\n")
		...
		match (m_label.align):
			Camera2D.INLINE_ALIGNMENT_CENTER:
